package co.yap.widgets.advrecyclerview.decoration
...
class StickyHeaderItemDecoration : RecyclerView.ItemDecoration() {
    ...
    private fun createHeaderView(position: Int, parent: RecyclerView): View? {
        ...
        if (adapter.onCreateViewHolder(parent, viewType) is IStickyHeaderViewHolder) {
    ...
    private fun findNextHeaderView(parent: RecyclerView): View? {
        ...
        for (i in 1..visibleCount) {
            ...
            if (header != null && parent.getChildViewHolder(header) as? IStickyHeaderViewHolder != null) {
                return header
            }
        }
        return null
